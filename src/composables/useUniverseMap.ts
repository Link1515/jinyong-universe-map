...
export function useUniverseMap() {
  ...
  const selectedCharacter = computed(() =>
    state.detail.kind === "character" ? characterMap.get(state.detail.id) ?? null : null
  );
  const selectedRelationship = computed(() =>
    state.detail.kind === "relationship" ? relationshipMap.get(state.detail.id) ?? null : null
  );
    ...
    return relationships
      .filter(
        (relationship) =>
          ...
          (relationship.source === selectedCharacter.value?.id || relationship.target === selectedCharacter.value?.id)
      )
      .map((relationship) => ({
        ...relationship,
